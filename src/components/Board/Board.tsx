...
function Board() {
  ...
  function onBoardMouseDownHandler(
    ...
    dragStart.x = event.nativeEvent.clientX;
    dragStart.y = event.nativeEvent.clientY - headerHeight.current;
  ...
  function onMouseMoveHandler(event: SyntheticEvent<HTMLDivElement, MouseEvent>) {
    if (!isDragged || !treeRef.current || ! rect) {
      return;
    }

    const deltaX = (event.nativeEvent.clientX - dragStart.x) / viewportScale.scale;
    const deltaY = (event.nativeEvent.clientY - dragStart.y - headerHeight.current) / viewportScale.scale;
  
    treeRef.current.style.top = `${rect.top + deltaY}px`
    treeRef.current.style.left = `${rect.left + deltaX}px`
  }

  function onZoomHandler(option: Scale) {
    setViewportScale(option);
