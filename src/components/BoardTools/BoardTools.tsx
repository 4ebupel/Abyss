...
function BoardTools(props: Props) {
  ...
  const dropdownOptions = () => {
    return scaleOptions.map((option) => {
      return (
        <li
          className={styles['tools__dropdown-options']}
          key={option.scale}
  ...
  function zoomIn() {
    const idx = scaleOptions.findIndex((option) => option.scale === currentScale.scale);
  ...
  return (
    <div  className={styles['tools']}>
      ...
      <div className={styles['tools__scale']}>
        <button
          className={styles['tools__scale-btn']}
          onClick={zoomOut}
        >
